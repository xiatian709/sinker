...
type ClickHouseConfig struct {
	...
	// DrainTimeout caps how long Service.drain() will block during partition revoke.
	// When large backlogs cause drain to exceed Kafka's rebalance.timeout, the consumer
	// gets kicked out and a new revoke kicks in, producing infinite "consumer group cleanup"
	// logs. Capping drain lets the rebalance complete; uncommitted offsets get replayed.
	// In seconds; <=0 means use defaultDrainTimeout. Must be smaller than sarama rebalance.timeout.
	DrainTimeout int
}
...
const (
	...
	defaultDrainTimeout       = 240 // seconds; should be less than sarama rebalance.timeout
)
...
func (cfg *Config) Normallize() (err error) {
	...
	if cfg.Clickhouse.DrainTimeout <= 0 {
		cfg.Clickhouse.DrainTimeout = defaultDrainTimeout
	}
