...
// slowWriteThreshold flags individual CH writes that look slow. Useful for
// telling "CH is degraded" apart from "we're just busy".
...
//
// Important: this is invoked from Ring.genBatchOrShard while holding the ring
// mutex. If it blocks (e.g. on a saturated writing pool), the consumer goroutine
// that subsequently tries to acquire ring.mux can no longer make progress, the
// Kafka session goes silent, and the broker triggers a rebalance — that's
// exactly the "infinite consumer group cleanup" failure mode we hit under load.
//
// To break that coupling, Send tries non-blocking submission and, if the
// writing pool is saturated, drops the batch and advances the Kafka offset so
// the consumer stays responsive. The drop is counted in FlushMsgsDropTotal.
func (c *ClickHouse) Send(batch *model.Batch) {
	...
	accepted := util.GlobalWritingPool.TrySubmit(func() {
		c.loopWrite(batch)
		c.mux.Lock()
		c.numFlying--
		if c.numFlying == 0 {
	...
	// Writing pool saturated — drop the batch, advance Kafka offset, and let
	// the consumer breathe.
	statistics.FlushMsgsDropTotal.WithLabelValues(c.taskCfg.Name).Add(float64(rows))
	statistics.FlushMsgsErrorTotal.WithLabelValues(c.taskCfg.Name).Add(float64(rows))
	if commitErr := batch.Commit(); commitErr != nil {
		util.Logger.Warn("Batch.Commit failed after writing-pool drop",
			zap.String("task", c.taskCfg.Name),
			zap.Error(commitErr))
	}
	util.Logger.Warn("writing pool saturated, dropped batch to keep consumer alive",
		zap.String("task", c.taskCfg.Name),
		zap.Int("rows", rows),
		zap.Int("pool_backlog", util.GlobalWritingPool.Backlog()))
	c.mux.Lock()
	c.numFlying--
	if c.numFlying == 0 {
		c.taskDone.Broadcast()
...
func (c *ClickHouse) writeSeries(rows model.Rows, conn clickhouse.Conn) (err error) {
	...
	if len(seriesRows) != 0 {
		...
		c.mux.Unlock()
		util.Logger.Debug("ClickHouse.writeSeries succeeded", zap.Int("series", len(seriesRows)), zap.String("task", c.taskCfg.Name))
...
func (c *ClickHouse) write(batch *model.Batch, sc *pool.ShardConn, dbVer *int) (err error) {
	...
	statistics.WritingDurations.WithLabelValues(c.taskCfg.Name, c.taskCfg.TableName).Observe(cost.Seconds())
	if cost > slowWriteThreshold {
		util.Logger.Warn("slow ClickHouse write",
			zap.String("task", c.taskCfg.Name),
			zap.String("table", c.taskCfg.TableName),
			zap.Int("rows", len(*batch.Rows)),
...
// loopWrite writes one batch to ClickHouse without retry. On write failure the
// batch is dropped, the loss is counted in statistics.FlushMsgsDropTotal, and we
// still advance the Kafka offset so the pipeline keeps moving instead of getting
// stuck re-trying the same broken batch (which previously caused infinite
// rebalance under heavy backlog).
//
// Hot path (write+commit OK) is identical to before — no extra allocations,
// locks, or atomic ops beyond the counter Inc.
func (c *ClickHouse) loopWrite(batch *model.Batch) {
	var err error
	var dbVer int
	sc := pool.GetShardConn(batch.BatchIdx)

	taskName := c.taskCfg.Name
	rowsInBatch := batch.RealSize

	if err = c.write(batch, sc, &dbVer); err != nil {
		if errors.Is(err, context.Canceled) {
			util.Logger.Info("ClickHouse.write failed due to the context has been cancelled", zap.String("task", taskName))
			return
		}
		util.Logger.Error("flush batch failed, dropping without retry",
			zap.String("task", taskName),
			zap.Int("rows", rowsInBatch),
			zap.Error(err))
		statistics.FlushMsgsErrorTotal.WithLabelValues(taskName).Add(float64(rowsInBatch))
		statistics.FlushMsgsDropTotal.WithLabelValues(taskName).Add(float64(rowsInBatch))
		// Advance Kafka offset so we don't get stuck re-trying this same batch.
		if commitErr := batch.Commit(); commitErr != nil &&
			!errors.Is(commitErr, context.Canceled) && !errors.Is(commitErr, io.ErrClosedPipe) {
			util.Logger.Warn("Batch.Commit after drop failed",
				zap.String("task", taskName),
				zap.Error(commitErr))
		}
		return
	...
	if err = batch.Commit(); err != nil {
		if errors.Is(err, context.Canceled) || errors.Is(err, io.ErrClosedPipe) {
			util.Logger.Warn("Batch.Commit failed due to the context has been cancelled", zap.String("task", taskName))
			return
		}
		util.Logger.Error("Batch.Commit failed after successful write, treating as drop",
			zap.String("task", taskName),
			zap.Int("rows", rowsInBatch),
			zap.Error(err))
		statistics.FlushMsgsDropTotal.WithLabelValues(taskName).Add(float64(rowsInBatch))
