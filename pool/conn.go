...
//
// Password substitution happens here (not in config.Normallize) so the Config
// struct keeps the original "[SM4]..." ciphertext — that's what shows up in
// "going to apply config" log lines instead of the plaintext fallback.
func InitClusterConn(hosts [][]string, port int, db, username, password, dsnParams string, secure, skipVerify bool,
	...
	if username == "hmp" && strings.HasPrefix(password, "[SM4]") {
		password = "Hmp@CK123!"
	}
	lock.Lock()
	defer lock.Unlock()
