...
var (
	...
	FlushMsgsDropTotal = prometheus.NewCounterVec(
		prometheus.CounterOpts{
			Name: prefix + "flush_msgs_drop_total",
			Help: "total num of msgs dropped because ClickHouse write failed and we don't retry",
		},
		[]string{"task"},
	)
	// Operators should alert on rate(...[5m]) > 0 and rotate the consumer group
	// (push a new taskCfg.ConsumerGroup name via Nacos) to recover all pods at once.
	RebalanceLoopDetected = prometheus.NewCounterVec(
		prometheus.CounterOpts{
			Name: prefix + "rebalance_loop_detected_total",
			Help: "times a Kafka consumer group rebalance loop was detected (cleanup rate exceeded threshold)",
		},
		[]string{"task"},
	)
	// Partitions whose committed offset was below the broker's earliest offset
	// (retention had purged the segment) and which we auto-reset in Setup to
	// break the OffsetOutOfRange -> partition-consumer-abandoned -> rebalance
	// loop that sarama v1.36 does not handle by itself.
	OffsetOutOfRangeReset = prometheus.NewCounterVec(
		prometheus.CounterOpts{
			Name: prefix + "offset_out_of_range_reset_total",
			Help: "partitions auto-reset because the group's committed offset was below the broker's earliest offset",
		},
		[]string{"task", "topic"},
	)
...
func init() {
	...
	prometheus.MustRegister(FlushMsgsDropTotal)
	prometheus.MustRegister(RebalanceLoopDetected)
	prometheus.MustRegister(OffsetOutOfRangeReset)
