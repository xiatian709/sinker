...
func (ring *Ring) QuitIdle() {
	if ring.isIdle {
		...
		util.Logger.Debug(fmt.Sprintf("topic %s partition %d quit idle", ring.service.taskCfg.Topic, ring.partition), zap.String("task", ring.service.taskCfg.Name))
...
func (ring *Ring) MakeRoom(newMsg *model.InputMessage) {
	...
	if newMsg.Offset != ring.ringGroundOff+ring.ringCap ||
		...
		util.Logger.Warn(fmt.Sprintf("Ring.MakeRoom discarded %d messages for topic %v patittion %d, offset [%d,%d)",
	...
	if !ring.isIdle {
		if ring.ringFilledOffset > ring.ringGroundOff {
			...
			if ring.idleCnt >= 2 {
				...
				ring.isIdle = true
				ring.ringBuf = nil
				util.Logger.Debug(fmt.Sprintf("topic %s partition %d became idle", taskCfg.Topic, ring.partition), zap.String("task", taskCfg.Name))
				return
			}
		}
		ring.scheduleForceBatchOrShard()
...
func (ring *Ring) genBatchOrShard() {
	...
	if atomic.LoadUint32(&ring.service.state) != util.StateRunning {
		...
		if batch.RealSize > 0 {
			util.Logger.Debug(fmt.Sprintf("created a batch for topic %v patittion %d, offset [%d,%d), messages %d, parse errors: %d",
