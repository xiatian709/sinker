...
//
// To prevent infinite Kafka rebalance under heavy backlogs, drain is capped by
// Clickhouse.DrainTimeout. On timeout we return so the Kafka client can finish
// its revoke handshake; the background goroutine keeps draining and uncommitted
// offsets get replayed on the next session (at-least-once is preserved).
func (service *Service) drain() {
	...
	timeout := time.Duration(service.cfg.Clickhouse.DrainTimeout) * time.Second
	...
	case <-time.After(timeout):
		util.Logger.Warn("drain timed out, returning to let kafka rebalance complete",
			zap.String("task", service.taskCfg.Name),
			zap.Duration("timeout", timeout),
			zap.Int32("inflight", atomic.LoadInt32(&service.numFlying)))
	}
}
