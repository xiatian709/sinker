...
// Parsing is CPU-intensive JSON work — give it every available core. The
// previous formula capped workers at NumCPU/2, which left high-throughput
// pods starved of parsing capacity and ended up causing the upstream Kafka
// consumer to block on a full ring buffer.
...
// queueSize used to be 3 — a CH hiccup would immediately fill the queue and
// every Send would block, which in turn held ring.mux indefinitely and froze
// the Kafka consumer. Size now scales with workers to absorb bursts; combined
// with ClickHouse.Send's TrySubmit fast-fail, that lets the consumer keep
// flowing even when CH is degraded.
